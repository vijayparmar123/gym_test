...
if(mysqli_affected_rows($sql) > 0)
{
	...
	{ 
		$username=mysqli_real_escape_string($conn,$_REQUEST['username']); $update_user = $update_user . "username='$username', ";   
	}

	if(isset($_REQUEST['first_name']))
	{ 
		$first_name=mysqli_real_escape_string($conn,$_REQUEST['first_name']); $update_user= $update_user . "first_name='$first_name',"; 
	}
	if(isset($_REQUEST['last_name']))
	{ 
		$last_name=mysqli_real_escape_string($conn,$_REQUEST['last_name']); $update_user= $update_user . "last_name='$last_name',"; 
	}
	if(isset($_REQUEST['address']))
	{ 
		$address=mysqli_real_escape_string($conn,$_REQUEST['address']); $update_user= $update_user . "address='$address',";
	}
	if(isset($_REQUEST['email']))
	{ 
		$email=$_REQUEST['email']; $update_user= $update_user . "email='$email',";
	}
	if(isset($_REQUEST['mobile']))
	{ 
		$mobile=$_REQUEST['mobile']; $update_user= $update_user . "mobile='$email',";
	}
	if(isset($_REQUEST['birth_date']))
	{
		$birth_date=(int)$_REQUEST['birth_date']; $update_user= $update_user . "birth_date=$birth_date,";
	}
...
else
{
	...
	$result['error'] = "Opps! User profile not change..";
}
